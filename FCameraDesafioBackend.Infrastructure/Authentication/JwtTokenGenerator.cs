...
namespace FCameraDesafioBackend.Infrastructure.Authentication;
...
public class JwtTokenGenerator : IJwtTokenGenerator
{
    ...
    {
        return CreateToken(_jwtSettings.SecretRefresh, DateTime.UtcNow.AddDays(_jwtSettings.RefreshExpiresDays), id,
            email);
    }
    ...
    private (bool, string?) Validate(string token, string key)
    {
        ...
        return result.IsValid
            ? (result.IsValid, result.Claims[JwtRegisteredClaimNames.Email].ToString())
            : (false, null);
    }
